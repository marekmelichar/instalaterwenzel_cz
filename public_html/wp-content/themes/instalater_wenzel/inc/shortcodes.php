...
// [ buttons ]
add_shortcode('button', 'mm_button_shortcode');
function mm_button_shortcode( $atts, $text = 'enter text' )
{
	// set defaults
...
function simple_gallery_shortcode( $atts, $text = 'enter text' )
{
	...
	foreach ( $media as $img )
	{
		...
		// $html .= '<div class="'. esc_attr( $atts['img_wrap_class'] ) .'"><img src="'. esc_url( wp_get_attachment_image_url($img->ID, 'full') ) .'"
		// 			class="'. esc_attr( $atts['img_class'] ) .'"
		// 			alt="'. esc_attr( $img->post_title ) .'"></div>';
	}
	$html .= '</div>';
